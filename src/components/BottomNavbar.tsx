...
export const BottomNavbar: React.FC<BottomNavbarProps> = ({
  ...
  return (
    <nav className="w-full max-w-md mx-auto bg-stone-950/85 backdrop-blur-xl border-t border-white/10 text-white px-3 py-2 flex items-center justify-around z-40 shadow-[0_-10px_25px_rgba(0,0,0,0.5)]">
      ...
      {/* Secret Admin Commission Button */}
      {onOpenAdminCommission && (
        <button
          onClick={onOpenAdminCommission}
          className="flex flex-col items-center space-y-1 text-amber-400 hover:text-amber-300 transition-all"
          title="Panel Privado de Comisiones (Solo Dueño)"
        >
