...
import { Video, Mic, MicOff, VideoOff, Radio, Users, Gem, Gift, Send, Play, X, Maximize2, Minimize2 } from 'lucide-react';
...
export const UserLiveBroadcaster: React.FC<UserLiveBroadcasterProps> = ({
  ...
  // Live Comments Overlay State
  const [liveComments, setLiveComments] = useState<LiveComment[]>([
    { id: 'c1', username: 'sofi_fan', displayName: 'Sofía ✨', text: '¡Hola a todos en el live! 🔥', timestamp: '10:00' },
    { id: 'c2', username: 'alex_vip', displayName: 'Alex 🎮', text: '¡Saluda por favor creadora! 🙌', timestamp: '10:01' },
    { id: 'c3', username: 'juan_perez', displayName: 'Juan P.', text: '¡Listos para enviar regalos! 🚀', timestamp: '10:02' }
  ]);
  ...
  const [liveDiamonds, setLiveDiamonds] = useState(1500);
  ...
  // Viewer fluctuation interval & simulated incoming audience comments when live
  useEffect(() => {
    ...
    const commentTemplates = [
      '¡Me encanta tu transmisión! ❤️',
      '¡Sigue así creadora! 🔥',
      'Te envié un regalo especial ✨',
      '¡Saluda a México! 🇲🇽',
      '¡Qué gran contenido! 👏',
      '¡Te sigo desde hoy! ⭐'
    ];
  ...
  const handleStartLive = async () => {
    try {
      await fetch('/api/streamers/go-live', {
        method: 'POST',
  ...
  const handleSendBroadcasterComment = (e: React.FormEvent) => {
    ...
    const myComment: LiveComment = {
      ...
      displayName: 'Tú (Transmitiendo) 🎙️',
            ...
            <div className="relative z-10 p-3 flex flex-col justify-end space-y-2 mt-auto bg-gradient-to-t from-black/90 via-black/40 to-transparent max-h-56">
              <div className="space-y-1.5 overflow-y-auto max-h-40 pr-1 scrollbar-none">
                {liveComments.map((c) => (
                  ...
                  >
                    <span className={`font-black ${c.isHost ? 'text-yellow-400' : 'text-rose-400'}`}>
                      {c.displayName}:
                    </span>
